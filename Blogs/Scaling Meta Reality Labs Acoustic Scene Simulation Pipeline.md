...
This pipeline replaced an earlier tool that researchers were using to generate simulation data, one that topped out at around a million scenes per run and lacked the configuration flexibility teams actually needed. The redesign was built to scale to 10 million scenes — the architecture removes every bottleneck at that scale by design: simulation nodes scale horizontally with no cross-node coordination, storage is range-partitioned so writers never conflict, and metadata is accumulated locally and compacted in a single Presto step at the end. There is no fixed ceiling beyond that. The hard part was never the simulation itself — it was designing the infrastructure around it so that scale is a dial, not a wall.